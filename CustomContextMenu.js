(function insertingNecessaryCss() {
	var sheet = (function() {
		...
		style.appendChild(document.createTextNode(
			"html{width:100%;height:100%}body{width:100%;height:100%}.rightclickmenu{min-width:100px;max-width:300px;border-radius:4px;webkit-box-shadow:0 6px 12px rgba(0,0,0,.175);box-shadow:0 6px 12px rgba(0,0,0,.175);border:1px solid #74a9d8;color:#337ab7;background-color:white;z-index: 99}.rightitem{border-bottom:1px solid #e5e5e5;padding:3px 20px;line-height:1.42;font-size:medium;wrap:none}.rightclickmenu-mobile{width:80%;max-width:none;left:10%;top:20%;max-height:70%;overflow-y:scroll}.rightitem-mobile{font-size:xx-large}.rightitem:hover{background-color:#f5f5f5}"
		));
...
var CCmenuCoreObject = function(Element, menuNcallBack, index) {
	...
	(function init() {
				...
				if (item.constructor.toString().indexOf('Object') == -1) {
					...
					for (var key in item) {
						if (key == 'item') {
							...
							throw 'RIGHTCLICK_MODULE: ' + key + ' KEY of ' + (index + 1) +
								'th OBJECT in second arguments ARRAY is not valid, \n this OBJECT should contain only 2 keys: \n "item" and "callBack" \n ex: {item: "do something", callBack: function(){something;}}';
						}
					}
				}
			});
		...
		this.menu = (function() {
			...
			if (this.mobile) {
				menu.classList.add("rightclickmenu-mobile");
			}
			for (var i = 0; i < this.menuNcallBack.length; i++) {
				...
				if (this.mobile) {
					ask.classList.add("rightitem-mobile");
				}
